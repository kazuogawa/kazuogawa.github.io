---
name: loop-budget
description: Portfolio TriageとApproved Featureの推定トークン、self-throttle閾値、kill switchを各runの開始時と終了時に確認する。loop-constraintsの後、loop-triageまたはapproved-feature-loopの前に使用する。
---
...
2. 実行中のpatternを`daily-triage`または`approved-feature`として特定し、現在時刻から遡る24時間について同じpatternの`tokens_estimate`の合計を求める。
